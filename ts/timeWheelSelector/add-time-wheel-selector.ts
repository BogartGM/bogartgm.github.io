...
export function addTimeWheelSelector(inputTimeSelector: HTMLInputElement) {
  inputTimeSelector.addEventListener("mouseenter", () => {
    inputTimeSelector.addEventListener("wheel", (event) =>
      handleWheelEvent(event, inputTimeSelector)
    );
  });
