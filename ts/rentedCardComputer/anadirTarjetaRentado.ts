...
import { FormValues } from "../interfaces/formValues.js";
...
export function anadirTarjetaRentado() {
  ...
  const onStop = () => {
    ...
    const modalidad = document.getElementById("exampleModal") as HTMLDivElement;
    ...
    const tiempoPedidoRentado = modalidad.querySelector(
      "#tiempo-pedido-rentado"
    );
    const tiempoTransRentado = modalidad.querySelector(
      "#tiempo-transcurrido-rentado"
    );
    ...
    if (tiempoTransRentado) {
      const tiempoTrans = node.querySelector("#tiempo-restante") as HTMLInputElement;
      tiempoTransRentado.innerHTML = tiempoTrans.value;
    }
    if (cobrarRentado) {
      const cobrar = node.querySelector("#cobrar") as HTMLInputElement;
      cobrarRentado.innerHTML = cobrar.value;
    }

    const botonModal = template.content.firstElementChild as HTMLElement;
